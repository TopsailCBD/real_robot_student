...
def getMatrixFromQuaternion(base_orientation):
  if(np.sum(np.square(base_orientation)) == 0):
    base_orientation[-1] = 1
  x, y, z, w = (base_orientation / np.sqrt(np.sum(np.square(base_orientation))))
...
#   if(np.sum(np.square(base_orientation)) == 0):
#     base_orientation[-1] = 1
#   return R.from_quat(base_orientation).as_matrix()
...
class VelocityEstimator:
  ...
  def _compute_delta_time(self, robot_state):
    if self._last_timestamp == 0.:
      ...
      delta_time_s = self.robot._time_step / self.robot._action_repeat
    else:
  ...
  def update(self, robot_state):
    ...
    for leg_id in range(4):
      if foot_contact[leg_id]:
        ...
        joint_velocities = self.robot._motor_velocities[leg_id *
                                                       3:(leg_id + 1) * 3]
